...
- **Codex:** tries the `app-server` via JSON-RPC over stdio (`codex_live`); on failure falls back to the local session cache (`codex_cached` reads the last `token_count` in `$CODEX_HOME/sessions/*.jsonl`) and flags the downgrade in `details`. `codex_bin()` works around PATH: it ignores `/mnt/` shims (Windows) and looks up the Linux binary via nvm. Multi-account: extra sessions are stored as `codex/<name>/auth.json` with `cli_auth_credentials_store = "file"`. Each collect sets `CODEX_HOME` to that home. Refresh tokens are single-use, so a registered profile whose email matches the live CLI is collected from the live `~/.codex` (and the stored copy is overwritten from it) rather than from the fork. `mark_codex_standby()` flags the accounts that are not the one in use, and the account in use comes from `codex_active_email()`: among every Codex home on the machine (`~/.codex` on both sides of WSL, listed via the shared `machine_homes()`), the one used last — ranked by `history.jsonl`, appended on every prompt, since a token refresh rewrites `auth.json` on its own. That home's `auth.json` names the account **unless** its `config.toml` points `model_provider` at a loopback `base_url`: a local router (CLIProxyAPI) then picks the account itself, so the account comes from the proxy's credential dir (`~/.cli-proxy-api`, or its `auth-dir`): the enabled entries ranked by `priority` (`fill-first`, ties to the newest `last_refresh`), minus the ones parked in a quota cooldown — the proxy drops a `.cds` sidecar next to a credential when it fails over, and it counts as out only while its `next_retry_after` is ahead. The head of that list is the account in use, so the badge follows the failover, not just the configured priority. Unregistered live sessions keep `account = "ChatGPT"`.
